...
The result is a $3\times2$ matrix filled by column using the numbers one through six. In other words, we have printed the transpose of the original matrix `t`, which we had defined in the global environment. Since `t` is a defined matrix and not a function, `R` follows the search path from the global environment to the parent, which is simply `package:base`. In the base environment, the function `t()` returns the transpose of the given matrix.
...
As expected, printing `T` returns a $2\times3$ matrix filled by row using the numbers one through six. Now let's enter `T` into the function `t`: 
...
Since `t` is a function we have defined in our workspace, `t(T)` takes `T` as an argument input and returns a column vector containing the numbers one through six. **NOTE: I'm having trouble explaining exactly why t(T) returns a column vector** Let's now see what would happen if we had defined `T` instead as `t`:
...
Since two objects cannot have the same name within one environment, the new matrix `t` overwrites the original function. `t` is now a defined matrix and no longer a function, so the search path moves to the base environment, where `t` is defined as the transpose function seen earlier. It is therefore clear why we receive the same result as in the previous exercise when printing `t(t)` here.  

This entire concept can be referred to as *name masking*. We can think of the transpose function `t` in the base environment as the "original" function. Each time a new object `t` is created in future environments, the original is "masked", but not overwritten. So if the search path is led back to the base environment, the original function can still be located. 
