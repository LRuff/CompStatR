...
\StringTok{'%or%'} \NormalTok{<-}\StringTok{ }\NormalTok{function(a, b) \{}
  \CommentTok{# Check if vectors a and b are logical}
  \NormalTok{if (!(}\KeywordTok{is.logical}\NormalTok{(a) &}\StringTok{ }\KeywordTok{is.logical}\NormalTok{(b))) \{}
    \KeywordTok{stop}\NormalTok{(}\StringTok{"a and b have to be logical vectors."}\NormalTok{)}
  \NormalTok{\}}
  
  \CommentTok{# Use ifelse to calculate the result and return it directly.}
...
\CommentTok{# Check the class of object t in the global environment}
\KeywordTok{class}\NormalTok{(.GlobalEnv$t)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Define t as the following matrix using the t function from above}
\NormalTok{t <-}\StringTok{ }\KeywordTok{t}\NormalTok{(}\DecValTok{1}\NormalTok{:}\DecValTok{6}\NormalTok{, }\DataTypeTok{ncol =} \DecValTok{3}\NormalTok{, }\DataTypeTok{byrow =} \OtherTok{TRUE}\NormalTok{)}

\CommentTok{# Check the class of object t in the global environment again}
\KeywordTok{class}\NormalTok{(.GlobalEnv$t)}
\end{Highlighting}
\end{Shaded}
